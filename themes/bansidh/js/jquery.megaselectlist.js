(function($){
	...
	{
		var config={classmodifier:"megaselectlist",headers:"rel",animate:false,animateevent:"mouseover",multiple:false,maximumitems:0,warningmessage:"You can only select {0} items",itemseparator:", ",hideloading:false,isxhtml:false};
		if(settings){
...
var replacementHtml='<div id="'+ classModifier+ nextModifierNumber+'" class="'+ classModifier+'">'+'<p tabindex="0"><span class="form-control"></span></p>'+'<div class="'+ classModifier+'options">';
var currentHeader="";
...
replacementHtml+='<div style="clear: both;">&nbsp;</div></div>'+'<div style="display: none;"><select name="'+ originalName+'"  id="'+ originalId+'" multiple="true" size="5"><option value="'+ selectedValue+'"'+ selectedAttribute+'>Selected</option></div>';
$This.remove();
$(label).hide().after(replacementHtml);
$(label).remove();
var spanText=" -Catégories-  ";
$("#"+ classModifier+ nextModifierNumber+" li.currentitem").each(function(){$("#"+ originalId).append('<option value="'+ $(this).attr("rel")+'"'+ selectedAttribute+'>'+ $(this).text()+'</option>');
...
$("#"+ classModifier+ nextModifierNumber+" li").keypress(function(e){
	if(e.keyCode=="13"||e.keyCode=="0"){e.preventDefault();
		$(this).trigger("click");}else if(e.keyCode=="33"){e.preventDefault();
		...
		else if(e.keyCode=="27"&&config.animate){e.preventDefault();
			$("#"+ classModifier+ nextModifierNumber+" .megaselectlistoptions").animate({height:"0px"});
			$("#"+ classModifier+ nextModifierNumber+" p:first")[0].focus();$("#"+ classModifier+ nextModifierNumber+" li").attr("tabindex","-1");
		}else{}});
$("#"+ classModifier+ nextModifierNumber+" li").click(function(){
	var item=$(this);
	var thisValue=$(item).attr("rel");
	if(!config.multiple){$(item).parents("."+ classModifier).find(".currentitem").removeClass("currentitem");
}
...
if(config.animate&&!config.multiple){
	...
	$(item).parent().parent().parent().parent().find("p:first")[0].focus();$(item).parent().parent().parent().parent().find("li").attr("tabindex","-1");
}
